...
function simulatedData = runTwoSeriesLinkSim()
...
calculatedData = zeros(length(K), length(p));
    ...
    for x = 1 : length(K)
         for y = 1 : length(p)
             ...
             calculatedData(x,y) = K(x) / (1 - p(y)) + K(x) / (1 - p(y)); %double the chance of failure in a double link network
         end
    end
...
function result = createData(K,p,N)
    ...
    for i=1:N
        ...
        while pktSuccessCount < K
            ...
            %second link transmission
            r = rand; % generate random number to determine if packet is successful (r > p)
            txAttemptCount = txAttemptCount + 1; % count 2nd attempt
